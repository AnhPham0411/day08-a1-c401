...
def get_embedding(text: str) -> List[float]:
    ...
    Hỗ trợ OpenAI và Sentence Transformers (Local) dựa trên .env
    """
    ...
    if provider == "openai":
        ...
        if _embedding_model is None:
            ...
            print(f"  [Info] Đang tải embedding model: {model_name}...")
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    for filepath in doc_files:
        ...
        # 3. Embed và lưu từng chunk vào ChromaDB
        ...
        for i, chunk in enumerate(tqdm(chunks, desc=f"Indexing {filepath.name}", leave=False)):
